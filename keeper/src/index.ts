...
async function pollRiskState() {
  try {
    const globalState = await client.program.account.globalState.fetch(globalStatePda);
    ...
    console.log(`[Keeper] Polled State | Risk Score: ${globalState.lastRiskScore.toNumber()} | CB Active: ${globalState.circuitBreakerActive}`);
    
    // If high risk and CB not active, we would normally trigger a proof here.
    // We'll mock the proof latency for Grafana metrics.
    if (globalState.lastRiskScore.toNumber() > 7000 && !globalState.circuitBreakerActive) {
