...
pub fn apply_shock(price: u64, shock_bps: u64) -> Result<u64, SimulatorError> {
    ...
    Ok(val / BPS_DENOMINATOR)
}

/// Apply a basis-points upward shock to a price.
pub fn apply_shock_up(price: u64, shock_bps: u64) -> Result<u64, SimulatorError> {
...
mod tests {
    ...
    fn test_30_percent_shock() {
        ...
        let shocked = apply_shock(price, 3000).unwrap();
    ...
    fn test_zero_shock() {
        let price = 150 * SCALE;
        let shocked = apply_shock(price, 0).unwrap();
    ...
    fn test_100_percent_shock() {
        let price = 150 * SCALE;
        let shocked = apply_shock(price, 10000).unwrap();
        assert_eq!(shocked, 0);
    }
    ...
    fn test_50_percent_shock() {
        let price = 200 * SCALE;
        let shocked = apply_shock(price, 5000).unwrap();
