...
const KEYPAIR_PATH = resolve(process.env.HOME || "~", ".config/solana/id.json");
const RESULTS_PATH = resolve(__dirname, "../outputs/results.json");
const PROOF_PATH = resolve(__dirname, "../outputs/sp1/proof.bin");
...
async function main() {
  ...
  if (existsSync(PROOF_PATH)) {
    ...
    if (proofBytes.length > 800) {
      console.log(
        "⚠ Proof is too large for a single tx. Truncating for mock verification.",
      );
  ...
  if (!existsSync(PUBLIC_VALUES_PATH)) {
    console.error(
      "❌ public_values.bin not found. Run the prover first: ./scripts/5-prove.sh",
    );
    process.exit(1);
  }
  ...
  const preStateLog = await (program.account as any).globalState.fetch(
    globalStatePda,
  );
  console.log(
    "  On-chain stateHash: 0x" +
      Array.from(preStateLog.lastStateHash as number[])
  ...
  try {
    ...
    console.log(
      `✓ Proof submitted and verified on-chain (tx: ${tx.slice(0, 16)}...)`,
    );
